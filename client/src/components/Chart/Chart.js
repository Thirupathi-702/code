...
const Chart = ({ name }) => {
  ...
  const [endDate, setEndDate] = useState(new Date('2024-08-18'));
    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.polygon.io/v2/aggs/ticker/${name}/range/1/day/${start}/${end}?adjusted=true&sort=asc&apiKey=U4qwOtOtN38xVCLt23myOo7L09kjkODC`
        );
        console.log(`https://api.polygon.io/v2/aggs/ticker/${name}/range/1/day/${start}/${end}?adjusted=true&sort=asc&apiKey=U4qwOtOtN38xVCLt23myOo7L09kjkODC`)
